defmodule SimpleWeb.PasswordResetControllerTest do
  use SimpleWeb.ApiCase, resource_name: :password_reset
  import Simple.TestEnvironmentHelper, only: [modify_env: 2]
  ...
  test "does not create resource and renders errors when password does not match", %{conn: conn} do
    ...
    assert %{"errors" => [%{"detail" => "Password_confirmation passwords do not match", "source" => %{"pointer" => "data/attributes/password-confirmation"}, "status" => "422", "title" => "passwords do not match"}]} = response
  end
  ...
  test "does not create resource and renders errors when error in token timeout occurs", %{conn: conn} do
    modify_env(:simple, password_reset_timeout: 0)
