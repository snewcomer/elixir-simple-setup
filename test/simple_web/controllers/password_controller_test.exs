defmodule SimpleWeb.PasswordControllerTest do
  @moduledoc false

  use SimpleWeb.ApiCase, resource_name: :password
  use Bamboo.Test
  ...
  test "Unauthenticated - creates and renders resource when email is valid", %{conn: conn} do
    ...
    assert_delivered_email Simple.Emails.ForgotPasswordEmail.create(user, token)
  end
  ...
  test "Authenticated - creates and renders resource when email is valid and removes session", %{conn: conn} do
    ...
    refute Simple.Guardian.Plug.authenticated?(conn)
  end
  ...
  test "does not create resource and renders 200 when email is invalid", %{conn: conn} do
    ...
    refute_delivered_email Simple.Emails.ForgotPasswordEmail.create(user, nil)
  end
