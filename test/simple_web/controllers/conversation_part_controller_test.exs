defmodule SimpleWeb.ConversationPartControllerTest do
  use SimpleWeb.ApiCase, resource_name: :conversation_part
  ...
  describe "create" do
    ...
    test "creates and renders resource when data is valid", %{conn: conn, current_user: user} do
      conversation = insert(:conversation, user: user)
      attrs = @valid_attrs |> Map.merge(%{user_id: user.id, conversation_id: conversation.id})
    ...
    test "does not create resource and renders 422 when data is invalid", %{
      ...
      conversation = insert(:conversation, user: user)
      attrs = @invalid_attrs |> Map.merge(%{user_id: user.id, conversation_id: conversation.id})
  ...
  describe "delete" do
    ...
    @tag :authenticated
    test "deletes conversation part", %{conn: conn, current_user: user} do
      conversation = insert(:conversation, user: user)
      conversation = insert(:conversation_part, conversation: conversation)
    ...
    test "renders 401 when not authenticated", %{conn: conn} do
      conversation = insert(:conversation_part)
