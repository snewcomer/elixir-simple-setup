defmodule Simple.MessagesTest do
  ...
  describe "conversations" do
    ...
    @valid_attrs %{body: "some body", is_locked: true, read_at: Timex.now(), receive_notifications: true, 
      status: "open", title: "some title"}
    @update_attrs %{body: "some updated body", is_locked: false, read_at: Timex.now(), receive_notifications: false, 
      status: "closed", title: "some updated title"}
    @invalid_attrs %{body: nil, is_locked: nil, read_at: nil, receive_notifications: nil, status: nil, title: nil, user_id: nil}
    ...
    test "create_conversation/1 with valid data creates a conversation" do
      user = insert(:user)
      params = %{
        body: "some body", is_locked: true, read_at: Timex.now(), receive_notifications: true, 
          status: "open", title: "some title", user_id: user.id
      }
      assert {:ok, %Conversation{} = conversation} = Messages.create_conversation(params)
  ...
  describe "conversation_parts" do
    ...
    def conversation_part_fixture() do
      conversation = insert(:conversation)
      user = insert(:user)
      attrs = Map.put_new(@valid_attrs, :user_id, user.id)
      attrs = Map.put_new(attrs, :conversation_id, conversation.id)
    ...
    test "list_conversation_parts/0 returns all conversation_parts" do
      ...
      assert Messages.list_conversation_parts(ConversationPart) == [conversation_part]
    end
    ...
    test "create_conversation_part/1 with valid data creates a conversation_part" do
      ...
      assert {:ok, %ConversationPart{} = conversation_part} = Messages.create_conversation_part(params)
      assert conversation_part.body == "some body"
      assert conversation_part.read_at
      assert conversation_part.user_id == user.id
